...
func (s *PostgreSQLStore) ListAuditEvents(ctx context.Context, params ListAuditParams) ([]AuditEvent, int64, error) {
	if params.Page < 1 {
		params.Page = 1
	}
	if params.Limit < 1 {
	...
	countQuery := rebindForPostgres(fmt.Sprintf("SELECT COUNT(*) FROM audit_events %s", whereClause)) // #nosec G201 -- whereClause built from constant strings; user values bound via args
	var total int64
	...
	dataQuery := rebindForPostgres(fmt.Sprintf(
		...
		whereClause,
	)) // #nosec G201 -- whereClause built from constant strings; user values bound via args
...
func (s *PostgreSQLStore) ListDistinctAuditActions(ctx context.Context) ([]string, error) {
