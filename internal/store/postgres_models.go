...
func (s *PostgreSQLStore) GetModelUsageSummary(ctx context.Context, modelName string) (*ModelUsageSummary, error) {
	const query = `SELECT
		COUNT(*),
...
// provider + upstream model combination.
func (s *PostgreSQLStore) GetModelUsageSummaryByUpstream(ctx context.Context, providerName, upstreamModel string) (*ModelUsageSummary, error) {
	const query = `SELECT
		COUNT(*),
		COALESCE(SUM(input_tokens), 0),
		COALESCE(SUM(cache_creation_input_tokens), 0),
		...
		COALESCE(MAX(model_upstream), '')
	FROM request_logs
	WHERE provider_name = $1 AND model_upstream = $2`
...
func (s *PostgreSQLStore) GetAllModelUsageSummaries(ctx context.Context) (map[string]*ModelUsageSummary, error) {
	const query = `SELECT
...
func (s *PostgreSQLStore) GetModelCostPricingGroups(ctx context.Context, modelName string) ([]CostPricingGroup, error) {
	const query = `SELECT
		model_requested,
	...
	FROM request_logs
	WHERE model_requested = $1
	GROUP BY model_requested, provider_name, model_upstream`
...
func (s *PostgreSQLStore) GetModelLatencyTimeseries(ctx context.Context, modelName string) ([]ModelLatencyTimeseriesEntry, error) {
	const query = `SELECT
		TO_CHAR(timestamp AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24') AS bucket,
		COALESCE(SUM(latency_ms), 0),
		COALESCE(SUM(output_tokens), 0),
		COUNT(*)
	FROM request_logs
	WHERE model_requested = $1 AND output_tokens > 0
	GROUP BY bucket
	ORDER BY bucket`
