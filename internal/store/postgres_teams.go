...
func (s *PostgreSQLStore) CreateTeam(ctx context.Context, id, name, description string) error {
	const q = `INSERT INTO teams (id, name, description, created_at) VALUES ($1, $2, $3, $4)`
...
func (s *PostgreSQLStore) ListTeams(ctx context.Context) ([]Team, error) {
	const q = `SELECT
...
func (s *PostgreSQLStore) ListTeamsWithMemberCounts(ctx context.Context) ([]TeamWithMemberCount, error) {
	const q = `SELECT
		t.id,
		t.name,
...
func (s *PostgreSQLStore) GetTeamByID(ctx context.Context, id string) (*Team, error) {
	const q = `SELECT
		t.id, t.name, COALESCE(t.description,''), t.created_at, tb.limit_usd, tb.period
	FROM teams t
	LEFT JOIN team_budgets tb ON tb.team_id = t.id
...
// pgScanTeam scans a Team from a *sql.Rows cursor.
func pgScanTeam(rows *sql.Rows) (*Team, error) {
	var t Team
	var budgetUSD sql.NullFloat64
	var budgetPeriod sql.NullString
...
func (s *PostgreSQLStore) DeleteTeam(ctx context.Context, teamID string) error {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM team_memberships WHERE team_id = $1`, teamID); err != nil {
...
func (s *PostgreSQLStore) AssignUserToTeam(ctx context.Context, userID, teamID string) error {
	const q = `INSERT INTO team_memberships (user_id, team_id, joined_at) VALUES ($1, $2, $3)
		ON CONFLICT (user_id) DO UPDATE SET team_id = EXCLUDED.team_id, joined_at = EXCLUDED.joined_at`
...
func (s *PostgreSQLStore) RemoveUserFromTeam(ctx context.Context, userID string) error {
	const q = `DELETE FROM team_memberships WHERE user_id = $1`
...
func (s *PostgreSQLStore) GetTeamMembership(ctx context.Context, userID string) (*TeamMembership, error) {
	const q = `SELECT user_id, team_id, joined_at FROM team_memberships WHERE user_id = $1`
...
func (s *PostgreSQLStore) ListTeamMembers(ctx context.Context, teamID string) ([]OIDCUser, error) {
